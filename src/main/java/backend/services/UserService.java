...
public class UserService {
    ...
    public ResponseEntity createUser(@Valid @RequestBody UserCreationDto user, Errors errors) {
        ...
        if (userRepository.findUserByUsername(user.getUsername()) != null)
            throw new UserHasNotBeenCreatedException(ExceptionMessages.ALREADY_EXISTS);
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(new UserCreationResponseDto(Response.MessageType.MESSAGE, ResponseMessages.USER_HAS_BEEN_CREATED, savedUser));
    }
    ...
    public ResponseEntity changePassword(@Valid @RequestBody PasswordChangeDto request, Errors errors, @RequestHeader Map<String, String> header) {
        if (errors.hasErrors())
            throw new BadCredentialsException(ExceptionMessages.VALIDATION_ERROR);
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new PasswordChangeResponseDto(Response.MessageType.MESSAGE, ResponseMessages.PASSWORD_HAS_BEEN_CHANGED));
    }
    ...
    public ResponseEntity showAllSignedUsers() {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new UserListResponseDto(Response.MessageType.STATUS, users));
    }
}
